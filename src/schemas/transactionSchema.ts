...
export const quoteExchangeSchema = z.object({
...
export const exchangeSchema = quoteExchangeSchema.extend({
  userAcceptedRate: z.number({ message: "La tasa de cambio aceptada (userAcceptedRate) es obligatoria." }).positive("La tasa de cambio aceptada debe ser mayor a cero."),
});

export const quoteBuySellSchema = z.object({
  currency: z.string({ message: "La moneda es obligatoria." }).trim().min(1, "La moneda es obligatoria.").toUpperCase(),
  amount: z.number({ message: "El monto debe ser un número mayor a cero." }).positive("El monto debe ser un número mayor a cero."),
});

export const buySellSchema = quoteBuySellSchema.extend({
  userAcceptedRate: z.number({ message: "La tasa de cambio aceptada (userAcceptedRate) es obligatoria." }).positive("La tasa de cambio aceptada debe ser mayor a cero."),
});
